...
CREATE POLICY event_banners_insert ON storage.objects FOR INSERT TO authenticated
WITH CHECK (
  bucket_id='event-banners' AND EXISTS(
    SELECT 1 FROM public.organisation_users ou
    WHERE ou.organisation_id=((storage.foldername(name))[1])::uuid AND ou.user_id=auth.uid() AND ou.status='active'
  )
);
DROP POLICY IF EXISTS event_banners_update ON storage.objects;
CREATE POLICY event_banners_update ON storage.objects FOR UPDATE TO authenticated
USING (bucket_id='event-banners' AND EXISTS(SELECT 1 FROM public.organisation_users ou WHERE ou.organisation_id=((storage.foldername(name))[1])::uuid AND ou.user_id=auth.uid() AND ou.status='active'))
WITH CHECK (bucket_id='event-banners' AND EXISTS(SELECT 1 FROM public.organisation_users ou WHERE ou.organisation_id=((storage.foldername(name))[1])::uuid AND ou.user_id=auth.uid() AND ou.status='active'));
DROP POLICY IF EXISTS event_banners_delete ON storage.objects;
CREATE POLICY event_banners_delete ON storage.objects FOR DELETE TO authenticated
USING (bucket_id='event-banners' AND EXISTS(SELECT 1 FROM public.organisation_users ou WHERE ou.organisation_id=((storage.foldername(name))[1])::uuid AND ou.user_id=auth.uid() AND ou.status='active'));
...
WITH CHECK(bucket_id='merchandise-images' AND EXISTS(SELECT 1 FROM public.organisation_users ou WHERE ou.organisation_id=((storage.foldername(name))[1])::uuid AND ou.user_id=auth.uid() AND ou.status='active'));
...
USING(bucket_id='merchandise-images' AND EXISTS(SELECT 1 FROM public.organisation_users ou WHERE ou.organisation_id=((storage.foldername(name))[1])::uuid AND ou.user_id=auth.uid() AND ou.status='active'))
WITH CHECK(bucket_id='merchandise-images' AND EXISTS(SELECT 1 FROM public.organisation_users ou WHERE ou.organisation_id=((storage.foldername(name))[1])::uuid AND ou.user_id=auth.uid() AND ou.status='active'));
...
-- ============================================================
-- COMPACT V21: SEASON-BASED TEAM SUBSCRIPTIONS
-- ============================================================
...
-- ClubOS Compact v37 targeted enhancements
