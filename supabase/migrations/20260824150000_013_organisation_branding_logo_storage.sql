-- ClubOS v7: organisation logo storage and branding support

-- Ensure every organisation has a branding record.
INSERT INTO public.organisation_branding (organisation_id)
SELECT o.id
FROM public.organisations o
WHERE NOT EXISTS (
  SELECT 1 FROM public.organisation_branding b WHERE b.organisation_id = o.id
);

-- Public bucket is intentional for organisation logos that are rendered in portals,
-- receipts and other club-facing surfaces. Writes remain protected by RLS below.
INSERT INTO storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
VALUES (
  'organisation-branding',
  'organisation-branding',
  true,
  5242880,
  ARRAY['image/png','image/jpeg','image/webp','image/svg+xml']
)
ON CONFLICT (id) DO UPDATE SET
  public = EXCLUDED.public,
  file_size_limit = EXCLUDED.file_size_limit,
  allowed_mime_types = EXCLUDED.allowed_mime_types;

DROP POLICY IF EXISTS "organisation_branding_public_read" ON storage.objects;
CREATE POLICY "organisation_branding_public_read"
ON storage.objects FOR SELECT
TO public
USING (bucket_id = 'organisation-branding');

DROP POLICY IF EXISTS "organisation_branding_member_insert" ON storage.objects;
CREATE POLICY "organisation_branding_member_insert"
ON storage.objects FOR INSERT
TO authenticated
WITH CHECK (
  bucket_id = 'organisation-branding'
  AND public.user_in_org((storage.foldername(name))[1]::uuid)
);

DROP POLICY IF EXISTS "organisation_branding_member_update" ON storage.objects;
CREATE POLICY "organisation_branding_member_update"
ON storage.objects FOR UPDATE
TO authenticated
USING (
  bucket_id = 'organisation-branding'
  AND public.user_in_org((storage.foldername(name))[1]::uuid)
)
WITH CHECK (
  bucket_id = 'organisation-branding'
  AND public.user_in_org((storage.foldername(name))[1]::uuid)
);

DROP POLICY IF EXISTS "organisation_branding_member_delete" ON storage.objects;
CREATE POLICY "organisation_branding_member_delete"
ON storage.objects FOR DELETE
TO authenticated
USING (
  bucket_id = 'organisation-branding'
  AND public.user_in_org((storage.foldername(name))[1]::uuid)
);
