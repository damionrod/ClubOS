...
export function AddMember() {
  ...
  const [form,setForm] = useState({ first_name:'', last_name:'', preferred_name:'', email:'', mobile:'', date_of_birth:'', address_line1:'', suburb:'', city:'', region:'', postcode:'', country:'New Zealand', status:'active', membership_type_id:'', team_id:'', emergency_name:'', emergency_relationship:'', emergency_mobile:'', medical_conditions:'', allergies:'', dietary_requirements:'', guardian_name:'', guardian_relationship:'Parent', guardian_email:'', guardian_mobile:'' });
  ...
  async function submit(e:FormEvent){
    ...
    try {
      ...
      if(form.team_id) { const selectedTeam=teams.find(t=>t.id===form.team_id); const {error}=await supabase.from('team_members').insert({organisation_id:activeOrg.id,team_id:form.team_id,member_id:member.id,season:selectedTeam?.season||null,role:'player'}); if(error) throw error; }
